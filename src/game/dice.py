...
def parse_dice_expression(expression: str) -> DiceInstruction:
    ...
    if not match:
        ...
        if ability_match:
            ability = ability_match.group(1)
            modifier = _extract_modifier(text.replace(ability, ""))
            return DiceInstruction(
                count=1, sides=20, modifier=modifier, advantage=0, ability=ability
            )
    ...
    return DiceInstruction(
        count=count, sides=sides, modifier=modifier, advantage=advantage, ability=ability
    )
...
def format_roll(result: RollResult) -> str:
    """Render a roll the way both Keith and the players should see it."""
    ins = result.instruction
    parts = [f"{ins.count}d{ins.sides}"]
    if ins.advantage == 1:
        parts.append("(advantage)")
    elif ins.advantage == -1:
        parts.append("(disadvantage)")

    detail = f"[{', '.join(str(r) for r in result.rolls)}]"
    if result.kept != result.rolls:
        detail += f" keep {result.kept[0]}"

    if ins.ability:
        parts.append(f"+ {ins.ability.upper()} {result.ability_modifier:+d}")
    elif result.ability_modifier:
        parts.append(f"{result.ability_modifier:+d}")
    if ins.modifier:
        parts.append(f"{ins.modifier:+d}")

    return f"{' '.join(parts)} → {detail} = **{result.total}**"
